...
function getRole(threadData, senderID) {
        ...
        if (rootOwner.includes(senderID))
                return 10;

        if (managerUsers.includes(senderID))
                return 9;

        if (supportUsers.includes(senderID))
                return 8;

        if (modUsers.includes(senderID))
                return 7;
